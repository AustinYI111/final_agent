...
class DataProviderConfig:
    ...
    polite_sleep_sec: float = 0.3
    chunk_sleep_sec: float = 0.5
...
class DataProvider:
    ...
    def __init__(
        ...
        adjust: str = "qfq",
        data_dir: str = "data/raw",
        cfg: Optional[DataProviderConfig] = None,
    ):
        ...
        self.cfg = cfg or DataProviderConfig()
    ...
    def _fetch_once_with_retry(self, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        def _tx_symbol(code: str) -> str:
            # 腾讯接口通常需要带市场前缀
            # 规则：51开头→上海(sh)，15/16开头→深圳(sz)，6开头→上海(sh)，其余→深圳(sz)
        ...
        try:
            ...
            # ---------- C) 东方财富ETF专用（针对ETF/基金类） ----------
            # 519925、510360 等是基金ETF代码，stock_zh_a_hist 对其支持不稳
            # fund_etf_hist_em 是东方财富专用ETF接口，更可靠
            try:
                time.sleep(self.cfg.polite_sleep_sec)
            ...
            try:
                ...
                # 新浪对ETF/基金可能返回无日期列的空数据，跳过
                if "date" not in df_sina.columns and "日期" not in df_sina.columns:
                    raise RuntimeError(f"sina returned no date column for ETF/fund {self.symbol}")
                return df_sina
            except (KeyError, RuntimeError) as e_key:
                # KeyError('date') = 新浪返回格式不兼容；RuntimeError = 空数据/无日期列
                last_err = e_key
...
# 兼容性别名（方便逐步迁移）
# -----------------------
DataAgent = DataProvider
DataAgentConfig = DataProviderConfig
